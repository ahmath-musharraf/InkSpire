...
// --- DATA VERSION ---
// IMPORTANT: To make changes to the initial data below (like updating image URLs),
// you MUST increment this version number. This tells the website to use your
// new code changes instead of the data saved in the user's browser.
// For example, change from 1 to 2.
...
const PRODUCT_CATEGORIES = ['Stationery & Office', 'Marketing & Promo', 'Signage & Display', 'Apparel & Textiles', 'Gifts & Personal'];

const initialProductsData = [
    { name: 'Premium Business Cards (x100)', price: 'LKR 2,500', description: '350gsm matte laminated cards with sharp, vibrant colors.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Stationery & Office' },
    { name: 'A5 Flyers (x500)', price: 'LKR 8,000', description: 'High-quality 150gsm gloss paper, perfect for promotions.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Marketing & Promo' },
    { name: 'Roll-Up Banner (Standard)', price: 'LKR 12,500', description: 'Durable, portable, and easy to set up for any event.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Signage & Display' },
    { name: 'Custom Mugs (x10)', price: 'LKR 4,500', description: 'Personalized ceramic mugs, ideal for corporate branding.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Gifts & Personal' },
    { name: 'A4 Branded Notepads (x25)', price: 'LKR 6,000', description: 'Professionally designed notepads for office or client use.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Stationery & Office' },
    { name: 'Event T-Shirts (x20)', price: 'LKR 15,000', description: 'High-quality cotton t-shirts with custom screen printing.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Apparel & Textiles' },
    { name: 'Die-Cut Stickers (x100)', price: 'LKR 3,000', description: 'Custom shape vinyl stickers, waterproof and durable.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Marketing & Promo' },
    { name: 'Canvas Tote Bags (x50)', price: 'LKR 20,000', description: 'Eco-friendly tote bags with your custom design printed.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Apparel & Textiles' },
    { name: 'Wall Calendars (A3, x10)', price: 'LKR 15,000', description: '12-page calendar, perfect for corporate new year gifts.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Gifts & Personal' },
    { name: 'Presentation Folders (x50)', price: 'LKR 9,500', description: 'A4 size folders with pockets, printed on 300gsm card.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Stationery & Office' },
    { name: 'Hardcover Journals (x10)', price: 'LKR 7,500', description: 'A5 hardcover journals with debossed or printed logos.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Stationery & Office' },
    { name: 'Embroidery Patches (x100)', price: 'LKR 10,000', description: 'High-quality custom embroidered patches for apparel.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Apparel & Textiles' },
    { name: 'PVC ID Cards (x50)', price: 'LKR 5,000', description: 'Durable, credit-card sized PVC cards for staff identification.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Stationery & Office' },
    { name: 'Branded Paper Bags (x100)', price: 'LKR 7,000', description: 'Custom printed paper bags for retail and events.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Marketing & Promo' },
    { name: 'Custom Lanyards (x100)', price: 'LKR 6,500', description: 'Satin finish lanyards with full-color sublimation printing.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Apparel & Textiles' },
    { name: 'Acrylic Keychains (x100)', price: 'LKR 9,000', description: 'Laser-cut acrylic keychains with your custom design.', image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=Product', category: 'Gifts & Personal' }
];
...
function loadData() {
    ...
    const savedVersion = parseInt(localStorage.getItem('inkSpireDataVersion') || '0', 10);

    // If the data version in the code is newer, load the fresh data from the code.
    // This ensures that updates made directly in `index.tsx` are applied.
    if (savedVersion < DATA_VERSION) {
        ...
        saveData(); // Save the new version and data immediately.
    } else {
        // Otherwise, load the data saved in the browser (which includes admin edits).
        const savedSiteData = localStorage.getItem('inkSpireSiteData');
        state.siteData = savedSiteData ? JSON.parse(savedSiteData) : JSON.parse(JSON.stringify(initialSiteData));
        
        const savedProducts = localStorage.getItem('inkSpireProducts');
        state.productsData = savedProducts ? JSON.parse(savedProducts) : JSON.parse(JSON.stringify(initialProductsData));
    }
}

function saveData() {
    localStorage.setItem('inkSpireDataVersion', DATA_VERSION.toString());
...
function renderProducts() {
    ...
    categories.forEach(category => {
        ...
        const button = createElement('button', {
            className: `px-6 py-2 rounded-full font-semibold transition-all duration-300 transform hover:scale-105 ${isActive ? 'bg-brand-accent text-brand-primary shadow-lg shadow-yellow-500/20' : 'bg-brand-secondary/50 text-gray-300 hover:bg-brand-secondary'}`,
    ...
    filterContainer.addEventListener('click', (e) => {
        ...
        if (target instanceof HTMLButtonElement && target.dataset.category) {
            state.activeProductCategory = target.dataset.category;
            renderProducts();
        }
    });
    ...
    filteredProducts.forEach((product, index) => {
        ...
        const productCard = createProductCard(product, originalIndex);
        productCard.style.setProperty('--animation-delay', `${index * 0.05}s`);
...
function createProductCard(product, index) {
  ...
  return createElement('div', {
    className: 'group',
    style: { animation: `fadeInUp 0.5s ease-out var(--animation-delay, 0s) forwards`, opacity: '0' }
  }, [
    createElement('div', {
        ...
        if(deleteBtn instanceof HTMLElement && deleteBtn.dataset.type && deleteBtn.dataset.index) {
            ...
            if (confirm(`Are you sure you want to delete this ${type}? This action is permanent and cannot be undone.`)) {
                if (type === 'product' && state.productsData[index]) {
                    ...
                    saveData();
                    renderProducts();
                } else if (type === 'faq' && state.siteData.faq[index]) {
                    state.siteData.faq.splice(index, 1);
                    saveData();
                    renderFAQ();
                }
            }
    ...
    document.getElementById('edit-form')?.addEventListener('submit', (e) => {
        ...
        if (type === 'product') {
            ...
            if (index !== null) {
                ...
                state.productsData.push({ ...updatedProduct, image: 'https://placehold.co/400x300/2a1a45/f0f0f0?text=New+Product' });
            }
            saveData();
            renderProducts();
            ...
            state.siteData.process[index] = updatedStep;
            saveData();
            renderPrintingProcess();
        } else if (type === 'faq') {
            const updatedFaq = {
            ...
            if (index !== null) {
                state.siteData.faq[index] = updatedFaq;
            } else {
                state.siteData.faq.push(updatedFaq);
            }
            saveData();
            renderFAQ();
        }
...
function openEditModal(type, index) {
    ...
    if (type === 'product') {
        ...
        data = isNew ? { category: PRODUCT_CATEGORIES[0] } : state.productsData[index];
        ...
        const categorySelect = createElement('select', {
            id: 'edit-category', name: 'category', required: true,
            className: 'w-full bg-brand-primary border border-brand-purple/50 rounded-md px-3 py-2 text-white focus:ring-brand-accent focus:border-brand-accent transition'
        });
        PRODUCT_CATEGORIES.forEach(cat => {
            const option = createElement('option', { value: cat }, [cat]);
        ...
        fieldsContainer.appendChild(
            createElement('div', {}, [
                createElement('label', { htmlFor: 'edit-category', className: 'block text-sm font-medium text-gray-300 mb-1' }, ['Category']),
                categorySelect
            ])
        );
        ...
        data = state.siteData.process[index];
        titleEl.textContent = 'Edit Process Step';
        fieldsContainer.appendChild(createField('title', 'Title', data.title));
        fieldsContainer.appendChild(createField('description', 'Description', data.description, 'textarea'));
    } else if (type === 'faq') {
        const isNew = index === null;
        data = isNew ? {} : state.siteData.faq[index];
