...
const state = {
    siteData: {
        images: {
            ...
            hero: 'https://raw.githubusercontent.com/ahmath-musharraf/InkSpire/refs/heads/main/inkspirelogo.png',
    ...
    const serviceCards = mainServices.map((service, index) => {
        return createElement('div', {
            ...
            createElement('div', { className: 'relative z-10 flex-grow' }, [
                createElement('div', { className: 'text-brand-accent mb-4 transition-transform duration-300 group-hover:rotate-12' }, [createSVG(service.icon)]),
...
function renderClients() {
    ...
    const clientLogos = clients.map(() => {
        return createElement('div', { className: 'flex items-center justify-center p-4 text-gray-500 hover:text-white transition-colors' }, [
    ...
    const content = createElement('div', { className: 'container mx-auto px-6' }, [
        createElement('div', { className: 'text-center mb-8' }, [
            createElement('h3', { className: 'text-lg text-gray-400 font-semibold' }, ['Trusted by leading businesses in the Eastern Province and beyond'])
        ]),
        createElement('div', { className: 'grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8' }, clientLogos)
    ]);
    container.appendChild(content);
}
...
function setupTestimonialsCarousel() {
    ...
    testimonials.forEach((_, index) => {
        const dot = createElement('button', {
            className: `w-3 h-3 rounded-full transition-colors ${index === 0 ? 'bg-brand-accent' : 'bg-gray-600'}`,
...
function renderFAQ() {
    ...
    const faqItems = state.siteData.faq.map((item, index) => {
        return createElement('div', { 
            ...
            createElement('div', {
                className: 'flex justify-between items-center cursor-pointer p-6 bg-brand-secondary/50 rounded-t-lg faq-question',
            }, [
                createElement('h4', { className: 'text-lg font-semibold text-white' }, [item.question]),
                createElement('div', { className: 'transition-transform duration-300' }, [createSVG(ICONS.CHEVRON_DOWN)])
            ]),
            createElement('div', {
                className: 'faq-answer max-h-0 overflow-hidden transition-all duration-500 ease-in-out',
            }, [
                createElement('p', { className: 'p-6 bg-brand-secondary rounded-b-lg text-gray-300' }, [item.answer])
            ])
        ]);
    });
    ...
    const content = createElement('div', { className: 'container mx-auto px-6' }, [
        ...
        createElement('div', { id: 'faq-accordion', className: 'max-w-3xl mx-auto' }, faqItems)
    ]);
    container.appendChild(content);
}
    ...
    container.appendChild(
        createElement('div', { className: 'container mx-auto px-6' }, [
            createElement('div', { className: 'bg-brand-accent rounded-lg shadow-lg p-12 text-center text-brand-primary' }, [
                ...
                createElement('a', {
                    href: '#contact',
                    className: 'bg-brand-primary text-brand-accent font-bold py-3 px-8 rounded-full hover:bg-white transition-colors text-lg'
                }, ['Contact Us'])
            ])
        ])
    );
...
function renderContactSection() {
    ...
    const contactForm = createElement('form', { className: 'space-y-6' }, [
        ...
        createElement('button', { type: 'submit', className: 'w-full bg-brand-accent text-brand-primary font-bold py-4 px-6 rounded-lg hover:bg-yellow-400 transition-colors' }, ["Send Message"])
    ]);
        ...
        createElement('div', { className: 'grid md:grid-cols-2 gap-16 items-center' }, [
            createElement('div', {}, [
                ...
                createElement('div', { className: 'space-y-6' }, [
                    createElement('a', { href: 'tel:+940742200156', className: 'flex items-center text-gray-300 hover:text-brand-accent' }, [createSVG(ICONS.PHONE), createElement('span', { className: 'ml-4' }, ["+94 (074) 220 0156"])]),
                    createElement('a', { href: 'mailto:inkspire.lk@gmail.com', className: 'flex items-center text-gray-300 hover:text-brand-accent' }, [createSVG(ICONS.EMAIL), createElement('span', { className: 'ml-4' }, ["inkspire.lk@gmail.com"])])
                ])
            ]),
            contactForm
        ])
...
function renderFooter() {
    ...
    ].map(social => createElement('a', { href: social.href, target: '_blank', rel: 'noopener noreferrer', className: 'text-gray-400 hover:text-brand-accent transition-colors' }, [
        createSVG(social.icon)
    ]));

    const content = createElement('div', { className: 'container mx-auto px-6 py-12' }, [
        createElement('div', { className: 'grid md:grid-cols-3 gap-8 text-center md:text-left items-start' }, [
            createElement('div', {}, [
            ...
            createElement('div', {}, [
                ...
                createElement('ul', { className: 'space-y-2' }, [
                    createElement('li', {}, [createElement('a', { href: '#services', className: 'text-gray-400 hover:text-brand-accent' }, ['Services'])]),
                    createElement('li', {}, [createElement('a', { href: '#products', className: 'text-gray-400 hover:text-brand-accent' }, ['Products'])]),
                    createElement('li', {}, [createElement('a', { href: '#contact', className: 'text-gray-400 hover:text-brand-accent' }, ['Contact'])] ),
                ])
            ]),
            createElement('div', {}, [
...
function setupEventListeners() {
    ...
    document.getElementById('faq-accordion')?.addEventListener('click', e => {
        ...
        const icon = question.querySelector('svg')?.parentElement;

        accordion?.querySelectorAll('.faq-answer').forEach(el => {
            if (el !== answer) {
        ...
        if (results.length > 0) {
            results.forEach(item => {
                const resultEl = createElement('a', {
                    href: item.link,
                    className: 'block p-4 hover:bg-brand-purple transition-colors border-b border-brand-purple/30',
                }, [
